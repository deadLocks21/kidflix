...
Définit le design system couleur Kidflix : la palette brute de tokens
(`KidflixPalette`), l'extension de thème `AppColors` pour les tokens custom non
couverts par Material `ColorScheme`, et la construction du `ThemeData` dark
appliqué à l'application via `AppThemeData.buildDarkTheme()`.
