...
Lecture d'un film depuis la `MovieDetailModal` jusqu'à la fin, avec
démarrage de la lecture dès que le téléchargement atteint le seuil de
buffer, reprise de lecture proposée par dialogue si une progression
existe, sauvegarde périodique de la progression, marquage complété à >
90% regardé, contrôles minimaux (play/pause, seek, fermer),
orientation landscape forcée sur mobile, mode immersive, wakelock actif.
Couvre le modèle Domain `WatchProgress`, le contrat
`WatchProgressRepository`, la page player, les widgets de contrôles et
de seek bar, le dialogue de reprise, et l'intégration depuis la modale
de détails. Ne couvre PAS le kid lock, les contrôles avancés (vitesse,
sous-titres, skip ±10s, chapitres), la synchronisation multi-device de
la progression, l'alimentation de la row `continueWatching` du home,
ni la gestion du stockage.
...
the overlay through a `MaterialVideoControlsTheme` with the following
constraints:
...
#### Scenario: Top bar shows close button and title
...
#### Scenario: Shortcut gestures are disabled

- **GIVEN** the player is active
