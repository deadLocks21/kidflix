...
- **GIVEN** a session in state `Authenticated` with profile `"leo"` having `hasPin == false`
- **WHEN** `SelectProfileUseCase.execute("leo")` is called
- **THEN** the session state becomes `ProfileSelected(Profile("leo", ...), session)`
...
- **GIVEN** a session in state `Authenticated` with profiles `["leo", "papa", "mia"]`
