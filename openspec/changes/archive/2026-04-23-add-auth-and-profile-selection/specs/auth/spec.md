...
The system SHALL generate a UUID v4 `device_id` at the first application startup on a given installation, and persist it in secure storage.

On subsequent startups, the system SHALL read the existing `device_id` from secure storage and reuse it.
...
Repository implementations (InMemory, SecureStorage) SHALL live exclusively in `lib/infrastructure/`.
...
- **THEN** no file under `lib/core/domain/` contains an import of `package:flutter/...`, `package:riverpod/...`, `package:dio/...`, `package:http/...`, or `package:flutter_secure_storage/...`
