...
void main() {
  group('InMemoryCatalogRepository.searchMovies', () {
    ...
    test(
      'accent-insensitive match (query without accents matches title with)',
      () async {
        // "Agent 327 : Opération Barbershop" carries an accent on "é".
        final results = await repo.searchCatalog(query: 'operation');
        expect(results.map((m) => m.id), contains('agent-327-barbershop'));
      },
    );

    test(
      'accent-insensitive match (query with accents matches title without)',
      () async {
    ...
    test(
      'returns matches across all age categories (no hierarchy filter)',
      () async {
        ...
        expect(
          ...
          greaterThan(1),
          reason:
              'in-memory mode must NOT apply an age-hierarchy filter — '
    ...
    test('query with no match returns empty list', () async {
      final results = await repo.searchCatalog(query: 'zzzzz-unlikely-title');
